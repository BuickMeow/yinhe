...
impl ApplicationDelegate {
    ...
    //
    // PATCHED (yinhe): AppKit may post `applicationDidFinishLaunching:` twice when an
    // Apple Event (e.g. Finder "open with") arrives before launch has fully finished;
    // the second call re-runs `dispatch_init_events` and crashes apps that create
    // windows in `Init`. Guard with the existing `is_launched` flag.
