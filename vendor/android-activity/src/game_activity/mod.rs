...
impl AndroidAppInner {
    ...
    // TODO: move into a trait
    //
    // yinhe 补丁：禁用 GameActivity 的 GameTextInput（白色原生输入框）。
    // egui TextEdit 聚焦时 egui-winit → winit → 本方法会把 GameActivity 的
    // textInputView 显示出来并请求弹键盘（SHOW_IMPLICIT，Android 12+ 无效），
    // 与应用自己的 IME 桥（隐藏 EditText + JNI）冲突。改为空操作：键盘的
    // 显示/隐藏、文本回流全部由应用侧 JNI 桥负责（yinhe-android/src/ime.rs）。
    pub fn show_soft_input(&self, _show_implicit: bool) {}

    // TODO: move into a trait
    //
    // yinhe 补丁：见上方 show_soft_input。
    pub fn hide_soft_input(&self, _hide_implicit_only: bool) {}
