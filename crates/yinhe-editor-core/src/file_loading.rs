//! 平台无关的文件加载逻辑。
//!
//! 负责后台线程加载（MIDI / .yin 工程 / 压缩包）、进度上报与结果轮询。
//! UI 层负责文件对话框（如 rfd）、i18n 文案，以及压缩包选择器 / 密码框等
//! 交互状态——压缩包多文件与密码缺失/错误的情况通过 [`LoadResult`] 的
//! `ArchivePickerNeeded` / `PasswordNeeded` 变体抛回给 UI 层处理。
...
pub enum LoadResult {
    ...
    /// 压缩包需要密码（`wrong_password == false`）或密码错误
    /// （`wrong_password == true`），需要 UI 层弹密码输入框。
    PasswordNeeded {
        path: String,
        wrong_password: bool,
    },
...
/// 进度条的 stage 文案由 UI 层提供（i18n）。
#[derive(Clone)]
pub struct LoadStageLabels {
    pub yin: String,
    pub archive: String,
    /// .yin 加载过程中“解压”阶段的 detail 文案。
    pub yin_decompress: String,
    /// .yin 加载过程中“重建”阶段的 detail 文案。
    pub yin_rebuild: String,
    /// .yin 加载过程中“重排”阶段的 detail 文案。
    pub yin_resort: String,
}

/// Manages async file loading (background thread + progress reporting).
...
impl FileLoader {
    pub fn new(load_progress: SharedProgress, labels: LoadStageLabels) -> Self {
        Self {
            midi_loader: None,
            yin_loader: None,
            archive_loader: None,
    ...
    pub fn is_loading(&self) -> bool {
        self.midi_loader.is_some() || self.yin_loader.is_some() || self.archive_loader.is_some()
    }
    ...
    fn start_yin(&mut self, path_str: String) {
        ...
        std::thread::spawn(move || {
            progress::set_stage_label(&progress, 0, yin);
            ...
            let result = yinhe_yin::load_yin_with_sf_progress(&path_for_thread, |p| {
                let detail = match p.stage {
                    yinhe_yin::YinProgressStage::Decompress => yin_decompress.clone(),
                    yinhe_yin::YinProgressStage::Rebuild => yin_rebuild.clone(),
                    yinhe_yin::YinProgressStage::Resort => yin_resort.clone(),
                    _ => String::new(),
                };
    ...
    pub fn start_archive(&mut self, path_str: String, password: Option<String>) {
        ...
        let path_for_thread = path_str.clone();
        let progress = self.load_progress.clone();
        let LoadStageLabels { archive, .. } = self.labels.clone();
        std::thread::spawn(move || {
            progress::set_stage_label(&progress, 0, archive);
            progress::set_stage(&progress, 0, StageStatus::Active);
            let result =
                ...
                match event {
                    ArchiveLoadEvent::Complete(result) => match result {
                        Ok((archive, entries)) => {
                            ...
                            // 多个 MIDI 文件：构造已就绪的 channel，交给 UI 层弹选择器
                            let (tx, rx) = mpsc::channel();
                            let _ = tx.send(Ok((archive, entries)));
                            return LoadResult::ArchivePickerNeeded {
                        ...
                        Err(e) => {
                            ...
                            // 密码错误/缺失：抛回 UI 层弹密码输入框而非直接报错
                            match &e {
                                yinhe_archive::ArchiveError::PasswordRequired => {
                                    progress::set_visible(&self.load_progress, false);
                                    return LoadResult::PasswordNeeded {
                                        path: loader.path.clone(),
