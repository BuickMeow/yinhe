//! 实验：测量 postcard 序列化 5 个音符列（1.64 亿元素）的耗时。
...
fn main() {
    ...
    let b = postcard::to_stdvec(&delta).unwrap();
    println!(
        "delta 序列化: {:?} -> {} MiB",
        t.elapsed(),
        b.len() / (1 << 20)
    ...
    let t = std::time::Instant::now();
    let b = postcard::to_stdvec(&key).unwrap();
    println!(
        "key 序列化: {:?} -> {} MiB",
        t.elapsed(),
        b.len() / (1 << 20)
    ...
    let t = std::time::Instant::now();
    let b = postcard::to_stdvec(&track).unwrap();
    println!(
        "track 序列化: {:?} -> {} MiB",
        t.elapsed(),
        b.len() / (1 << 20)
    ...
    let t = std::time::Instant::now();
    let b = postcard::to_stdvec(&vel).unwrap();
    println!(
        "vel 序列化: {:?} -> {} MiB",
        t.elapsed(),
        b.len() / (1 << 20)
    ...
    let t = std::time::Instant::now();
    let b = postcard::to_stdvec(&gate).unwrap();
    println!(
        "gate 序列化: {:?} -> {} MiB",
        t.elapsed(),
        b.len() / (1 << 20)
