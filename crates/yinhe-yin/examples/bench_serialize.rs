...
fn main() {
    ...
    let delta: Vec<u32> = (0..n)
        .map(|i| if i % 128 == 0 { ((i / 128) % 480) as u32 } else { 0 })
        .collect();
    ...
    println!("delta 序列化: {:?} -> {} MiB", t.elapsed(), b.len() / (1 << 20));
    let t = std::time::Instant::now();
    let b = opt.serialize(&key).unwrap();
    println!("key 序列化: {:?} -> {} MiB", t.elapsed(), b.len() / (1 << 20));
    let t = std::time::Instant::now();
    let b = opt.serialize(&track).unwrap();
    println!("track 序列化: {:?} -> {} MiB", t.elapsed(), b.len() / (1 << 20));
    let t = std::time::Instant::now();
    let b = opt.serialize(&vel).unwrap();
    println!("vel 序列化: {:?} -> {} MiB", t.elapsed(), b.len() / (1 << 20));
    let t = std::time::Instant::now();
    let b = opt.serialize(&gate).unwrap();
    println!("gate 序列化: {:?} -> {} MiB", t.elapsed(), b.len() / (1 << 20));
}
