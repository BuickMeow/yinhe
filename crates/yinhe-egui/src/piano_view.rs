...
pub fn show(
    ...
    *last_cursor_tick = *cursor_tick;
    ...
    // ── Scale background（调号驱动的调内/调外/根音条带）──
    ...
    // ── 八度横线（每个 C 位置一条细线）──
    {
        ...
        for key in (0u8..128).step_by(12) {
            ...
            painter.line_segment(
                [egui::pos2(content_left, screen_y), egui::pos2(content_rect.max.x, screen_y)],
...
///
/// 有调号时：调内音用背景色（不画），调外音用 `PR_SCALE_OUTSIDE` 暗色，根音用 `PR_ROOT_NOTE` 深蓝。
/// 无调号时：回退标准钢琴布局（黑键行用 `pr_black_key_row` 色带）。
///
/// 按可见 tick 范围内的调号事件分段，每段独立渲染。调号事件极少（通常 < 10），
/// 可见 key 也有限（~50），矩形数 < 500，性能可接受。
fn paint_scale_background(
    ...
    // 可见 key 范围（y_to_key 返回的 key 在可见区域顶部和底部）
