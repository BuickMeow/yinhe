...
use super::state::{EditRequest, NoteRef, TextEventKind};
...
pub(super) fn apply_keysig_popups(
    ui: &mut egui::Ui,
    doc: &mut Document,
    salt: &str,
) {
        ...
        PopupAction::Changed(new_tick) => {
            ...
            if new_tick != tick {
                ...
                if let Some(e) = model.conductor.key_sig.iter().find(|e| e.tick == tick) {
                    let _ = doc.set_keysig_event(tick, new_tick, e.sf, e.mi);
                }
    ...
    match action {
        PopupAction::Changed(new_sf) => {
            ...
            if new_sf != sf {
                let model = &doc.data.model;
                if let Some(e) = model.conductor.key_sig.iter().find(|e| e.tick == tick) {
                    let _ = doc.set_keysig_event(tick, tick, new_sf, e.mi);
                }
            }
        }
        PopupAction::Closed => {
    ...
    match action {
        PopupAction::Changed(new_mi) => {
            ...
            if new_mi != mi {
                let model = &doc.data.model;
                if let Some(e) = model.conductor.key_sig.iter().find(|e| e.tick == tick) {
                    let _ = doc.set_keysig_event(tick, tick, e.sf, new_mi);
                }
            }
        }
        PopupAction::Closed => {
...
// ────────────────────────────────────────────────────────────────
// Text event popups (Marker / Lyrics / Chord)
// ────────────────────────────────────────────────────────────────
...
pub(super) fn apply_text_popups(
    ui: &mut egui::Ui,
    doc: &mut Document,
    salt: &str,
) {
...
fn apply_text_event_edit(doc: &mut Document, kind: TextEventKind, old_tick: u32, new_tick: u32, new_text: String) {
    match kind {
        TextEventKind::Marker => { let _ = doc.set_marker_event(old_tick, new_tick, new_text); }
        TextEventKind::Lyrics { track } => { let _ = doc.set_lyrics_event(track, old_tick, new_tick, new_text); }
        TextEventKind::Chord { track } => { let _ = doc.set_chord_event(track, old_tick, new_tick, new_text); }
    }
}
