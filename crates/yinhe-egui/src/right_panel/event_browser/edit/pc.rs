    ...
    if let Some(req) = peek_pos_edit_request(ui, salt) {
        match req {
            EditRequest::PcTick { tick } => {
                show_pc_tick_popup(ui, doc, salt, track, tick, Some(bar_lookup))
            }
...
fn show_pc_program_popup(ui: &mut egui::Ui, doc: &mut Document, salt: &str, track: u16, tick: u32) {
    let program = doc
        .data
        .model
        .tracks
