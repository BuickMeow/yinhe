...
use super::{
    PopupAction, PopupConfig, cleanup_edit_request, push_event_list_undo, show_number_popup,
    show_tick_popup,
};
    ...
    if let Some(req) = peek_pos_edit_request(ui, salt) {
        match req {
            EditRequest::TimeSigTick { tick } => {
                show_timesig_tick_popup(ui, doc, salt, tick, Some(bar_lookup))
            }
...
fn show_timesig_num_popup(ui: &mut egui::Ui, doc: &mut Document, salt: &str, tick: u32) {
    let numerator = doc
        .data
        .model
        .conductor
...
fn show_timesig_den_popup(ui: &mut egui::Ui, doc: &mut Document, salt: &str, tick: u32) {
    let denominator = doc
        .data
        .model
        .conductor
