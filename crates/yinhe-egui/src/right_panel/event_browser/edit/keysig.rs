    ...
    if let Some(req) = peek_pos_edit_request(ui, salt) {
        match req {
            EditRequest::KeySigTick { tick } => {
                show_keysig_tick_popup(ui, doc, salt, tick, Some(bar_lookup))
            }
...
fn show_keysig_root_popup(ui: &mut egui::Ui, doc: &mut Document, salt: &str, tick: u32) {
    let root = doc
        .data
        .model
        .conductor
...
fn show_keysig_scale_popup(ui: &mut egui::Ui, doc: &mut Document, salt: &str, tick: u32) {
    let scale = doc
        .data
        .model
        .conductor
    ...
    let action = show_choice_popup(
        ui,
        salt,
        t!("event_browser.edit_keysig_scale").as_ref(),
        scale,
        ScaleType::ALL,
        |s| s.display_name().to_string(),
    );
    match action {
