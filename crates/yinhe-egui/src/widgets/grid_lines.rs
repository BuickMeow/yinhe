...
impl GridColors {
    /// Pianoroll 配色（automation 也用这套）：同色相四档透明度，整体加深。
    pub fn pianoroll() -> Self {
        let base = theme::line_fg();
        Self {
            measure: base,
            beat: egui::Color32::from_rgba_unmultiplied(base.r(), base.g(), base.b(), 140),
            sub_beat: Some(theme::grid_sub_beat()),
            tick: Some(theme::grid_tick()),
        }
    }

    /// Arrangement 配色（无 sub_beat / tick 线）：小节不透明、四分音符半透明以区分。
...
mod tests {
    ...
    /// 验证 GridColors 配色档位：同色相四档透明度，整体加深。
    #[test]
    fn test_grid_colors_distinct() {
        let pr = GridColors::pianoroll();
        let ar = GridColors::arrangement();
        // measure 255/beat 140/sub 110/tick 60 同 RGB 不同 alpha（±1 舍入）
        ...
        assert_eq!(pr.beat.a(), 140);
        assert_eq!(pr.sub_beat.unwrap().a(), 110);
        assert_eq!(pr.tick.unwrap().a(), 60);
