...
pub(crate) struct PendingEdits {
    map: HashMap<egui::Id, UndoSnapshot>,
}
...
impl PendingEdits {
    ...
    pub fn insert_raw(&mut self, id: egui::Id, snapshot: UndoSnapshot) {
        self.map.insert(id, snapshot);
    }

    pub fn take(&mut self, id: egui::Id) -> Option<UndoSnapshot> {
        self.map.remove(&id)
    }
}
...
pub(crate) fn begin_edit(data: &ProjectData, pending: &mut PendingEdits, id: egui::Id, label: &'static str) {
