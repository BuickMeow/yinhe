...
/// Shared data structure for track events grouped by (track, channel).
/// Used during both MIDI→archive and archive→MIDI conversions.
struct TrackEventGroup {
    notes: Vec<Note>,
...
pub fn midi_to_archive_with_names(
    ...
    let (group_to_new_idx, total_new_tracks) = assign_track_indices(&channels_per_track, num_tracks);
    let (new_track_names, new_track_ports, new_track_channel_prefixes) =
        build_track_metadata(midi, track_names, &channels_per_track, &group_to_new_idx, total_new_tracks);
...
/// Write conductor events (tempo, time signature) to the archive.
...
/// Group all notes and control events by (track_index, channel).
...
/// Discover which channels each original track uses.
...
/// Assign new track indices when a track uses multiple channels.
/// Returns (group→new_index mapping, total new track count).
...
/// Build metadata arrays for the new track layout.
...
/// Write archive entries for each (track, channel) group.
...
fn write_group_entries(
    ...
    for original_idx in 0..num_tracks {
        ...
        for &ch in channels {
            ...
            let (cc_events, rpn_events, pc_events) =
                classify_control_events(data);
            ...
            add_to_port_map(port_map, port, raw_channel, &uuid, name, new_idx, new_track_channel_prefixes[new_idx]);
        }
    }
}

/// Classify raw CC events into CC, RPN, and PC groups.
/// Extracts RPN sequences (CC 101/100/6/38) into dedicated RPN entries.
fn classify_control_events(
    data: &TrackEventGroup,
) -> (HashMap<u8, Vec<CcEvent>>, HashMap<u8, Vec<RpnEvent>>, Vec<PcEvent>) {
...
/// Add a track mapping entry to the port_map.
fn add_to_port_map(
    port_map: &mut HashMap<u8, Vec<(u8, Vec<TrackMapping>)>>,
    port: u8,
    raw_channel: u8,
...
/// Write mapping.json and project.json to the archive.
...
pub fn archive_to_midi(archive: &ProjectArchive) -> yinhe_midi::MidiFile {
    ...
    let (mut track_data, num_tracks) = read_track_data_from_entries(archive, &uuid_to_track, &mapping);
...
/// Read conductor events (tempo, time signature) from the archive.
...
/// Read track data from archive entries, grouped by track index.
...
/// Apply mapping metadata (names, ports, prefixes) to the midi file.
...
/// Rebuild key_notes and control_events arrays from track data.
...
/// Resolve the global channel for a track by scanning its archive entries.
...
/// Sort events and recompute derived fields.
...
/// Collect raw (tick, kind) events for a single track.
...
/// Build the list of TrackEvents with delta timing for one track.
...
/// Build the conductor track (tempo + time signature).
