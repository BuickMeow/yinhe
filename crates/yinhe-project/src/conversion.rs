    ...
    for ev in &midi.control_events {
        ...
        match ev {
            yinhe_midi::MidiControlEvent::ControlChange {
                ...
                data.cc_events.push((*controller, CcEvent { tick: *tick, value: *value }));
            }
...
pub fn export_midi(midi: &yinhe_midi::MidiFile, track_names: &[String], path: &str) -> Result<(), String> {
    let num_tracks = midi.track_ports.len();
    ...
    for track_idx in 0..num_tracks {
        ...
        if let Some(name) = track_names.get(track_idx) {
