...
fn color_event_midi() -> Vec<u8> {
    ...
    let track: &[u8] = &[
        ...
        0x81, 0x70, 0x80, 60, 0, // delta=240, NoteOff
        0x00, 0xFF, 0x2F, 0x00,
    ];
...
#[test]
fn color_event_parses_into_track_color() {
    let model = parse_bytes(&color_event_midi()).expect("parse failed");
...
/// 精确通道匹配应优先于 0x7F 通配。
#[test]
fn color_event_prefers_exact_channel_match() {
    let mut data = Vec::new();
    data.extend_from_slice(b"MThd");
    data.extend_from_slice(&6u32.to_be_bytes());
    data.extend_from_slice(&[0, 0, 0, 1, 1, 0xE0]);
    data.extend_from_slice(b"MTrk");
    let track: &[u8] = &[
        0x00, 0xFF, 0x0A, 0x08, 0x00, 0x0F, 0x00, 0x00, 0xFF, 0x00, 0x00, 0xFF, // ch=0 红
        0x00, 0xFF, 0x0A, 0x08, 0x00, 0x0F, 0x7F, 0x00, 0x00, 0x00, 0xFF, 0xFF, // ch=0x7F 蓝
        0x00, 0x90, 60, 100, // NoteOn channel 0
        0x81, 0x70, 0x80, 60, 0, 0x00, 0xFF, 0x2F, 0x00,
    ];
    data.extend_from_slice(&(track.len() as u32).to_be_bytes());
    data.extend_from_slice(track);
    ...
    assert_eq!(t.color[0], 1.0); // 精确匹配 ch=0 的红色
    assert_eq!(t.color[2], 0.0);
}
...
/// 渐变变体（12 字节 payload）只取第一组颜色。
#[test]
fn color_event_gradient_takes_first_color() {
    let mut data = Vec::new();
    data.extend_from_slice(b"MThd");
    data.extend_from_slice(&6u32.to_be_bytes());
    data.extend_from_slice(&[0, 0, 0, 1, 1, 0xE0]);
    data.extend_from_slice(b"MTrk");
    let track: &[u8] = &[
        0x00, 0xFF, 0x0A, 0x0C, 0x00, 0x0F, 0x7F, 0x00, 0x80, 0x40, 0x20, 0xFF, 0x10, 0x20, 0x30,
        0xFF, // 12 字节渐变：主色 (128,64,32)，渐变色 (16,32,48)
        0x00, 0x90, 60, 100, 0x81, 0x70, 0x80, 60, 0, 0x00, 0xFF, 0x2F, 0x00,
    ];
    data.extend_from_slice(&(track.len() as u32).to_be_bytes());
    data.extend_from_slice(track);
    ...
    let model = parse_bytes(&data).expect("parse failed");
    let t = &model.tracks[1];
    assert!((t.color[0] - 128.0 / 255.0).abs() < 0.01);
    assert!((t.color[1] - 64.0 / 255.0).abs() < 0.01);
    assert!((t.color[2] - 32.0 / 255.0).abs() < 0.01);
}
